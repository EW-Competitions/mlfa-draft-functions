...
def handle_carlos_perez(filtered_players, player_name, player_team):
    ...
    # The Carlos Perez from the Athletics was born on 1990-10-27
    # The Carlos Perez from the White Sox was born on 1996-09-10
    birthdate = '1990-10-27' if player_team == 'ATH' else '1996-09-10'

    return [p for p in filtered_players if p['birthdate']['raw'].startswith(birthdate)]
...
def get_player_with_id(headers, player_name, player_team, player_position):
    try:
        ...
        if response.status_code == 200:
            ...
            filtered_players = [p for p in results if unidecode(p['name']['raw']) == unidecode(player_name)]

            if player_name == 'Carlos Perez':
                filtered_players = handle_carlos_perez(filtered_players, player_name, player_team)

            if len(filtered_players) > 1:
                mapped_position = map_position(player_position, player_name)
                filtered_players = [p for p in filtered_players if p['position']['raw'] == mapped_position]

            if len(filtered_players) > 1:
                birthdate = ba_to_fg_birthdate_mapping[player_name] if player_name in ba_to_fg_birthdate_mapping else None
                if birthdate:
                    filtered_players = [p for p in filtered_players if p['birthdate']['raw'].startswith(birthdate)]
...
def load_players_with_ids(players):
    ...
    for player in players:
        player_with_id = get_player_with_id(headers, player['player'], player['team'], player['position'])
        if player_with_id:
